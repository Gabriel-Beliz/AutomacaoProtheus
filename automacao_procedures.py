...
class RoboProcedures():
        ...
        try:
            if self.entrada_pasta.get() != "" and self.senha_usuario_portal.get() != "" and self.entrada_usuario_portal.get() != "" and self.entrada_usuario_protheus.get() != "" and self.senha_usuario_protheus.get() != "":
                ...
                if self.validar_seleciona():
                    ...
                    label_procedures = tk.Label(
                        text="Finalizado!", font=fonte_subtitulos)
                    label_procedures.grid(
                        row=10, column=0, pady=5, sticky="nswe", columnspan=2)
                    self.navegador.close()
                    self.pw.stop()
            else:
                label_procedures = tk.Label(
                    text="Preencha todos os campos!", font=fonte_subtitulos)
                label_procedures.grid(
                    row=10, column=0, pady=5, sticky="nswe", columnspan=2)
        except:
            label_procedures = tk.Label(
                text="Ocorreu um Erro Interno.", font=fonte_subtitulos)
            label_procedures.grid(row=10, column=0, pady=5,
    ...
    def selecionar_pasta(self):
        try:
            pasta = filedialog.askdirectory(
                title="Selecione onde será salvo os Pacotes de Atualização")
            if pasta:
    ...
    def login(self):
        try:
            ...
            label_procedures=tk.Label(
                text="Erro no Login! Verificar Credenciais.", font=fonte_subtitulos)
            label_procedures.grid(row=10, column=0, pady=5,
                                  sticky="nswe", columnspan=2)
            self.navegador.close()
            self.pw.stop()
    ...
    def navegar_procedures(self):
        try:
            ...
            label_procedures=tk.Label(
                text="Ocorreu um Erro.", font=fonte_subtitulos)
            label_procedures.grid(row=10, column=0, pady=5,
                                  sticky="nswe", columnspan=2)
            self.navegador.close()
            self.pw.stop()
    ...
    def esperar_itens(self):
        try:
            self.itens=self.pagina.locator(
                "table tbody tr:has(td:nth-child(4))")
            ...
            for i in range(self.itens.count()):
                ...
                if "Não instalado" in texto or "Desatualizado" in texto:
                    ...
                    while (self.pagina.get_by_role("button", name="Cancelar").count()) == 0:
                        # Esperar abrir o grid com a visualização das atualizações
                        time.sleep(0.5)

                    botao=self.pagina.locator(
                        "wa-button[caption='Buscar atualização']:not([disabled])")
                    botao=botao.nth(0)


                    if botao.count() > 0 and "Não instalado" in texto:
                        ...
                        if 'https://' in valor:
                            ...
                            if self.xl == 0:
                                pagina2.get_by_role("textbox", name="Você está em um campo para login").fill(
                                    self.entrada_usuario_portal.get())
                                pagina2.get_by_role("textbox", name="Você está em um campo para senha").fill(
                                    self.senha_usuario_portal.get())
                                pagina2.get_by_role(
                                    "button", name="Botão para confirmação das").click()

                            with pagina2.expect_download() as download_info:
                                pagina2.get_by_role(
                                    "button", name="Fazer Download").click()

                            download=download_info.value
                            download.save_as(
                                f"{self.entrada_pasta.get()}/{download.suggested_filename}")

                            pagina2.close()
                            time.sleep(1)
                            fechar=self.pagina.get_by_role(
                                "button", name="fechar")
                            if fechar.count() > 0:
                                fechar.click()
                            time.sleep(3)
    ...
    def mov_direita_botao(self):
        try:
            ...
            label_procedures=tk.Label(
                text="Ocorreu um Erro. Entrar em Contato com o Desenvolvedor.", font=fonte_subtitulos)
            label_procedures.grid(row=10, column=0, pady=5,
                                  sticky="nswe", columnspan=2)
            self.navegador.close()
            self.pw.stop()
...
label_procedures=tk.Label(text="Atualização Procedures",
                          borderwidth=2, font=fonte_titulo)
label_procedures.grid(row=0, column=0, padx=15, pady=15,
                      sticky="nswe", columnspan=2)
...
botao_pasta=tk.Button(text="Selecionar Pasta",
                      command=robo_procedures.selecionar_pasta)
botao_pasta.grid(row=8, column=1, pady=5)

botao_executar=tk.Button(text="Executar", height=2,
                         command=robo_procedures.verifica_campos)
botao_executar.grid(row=9, column=0, columnspan=2,
                    padx=10, pady=20, sticky="nswe")
